...
class GoBackN(Protocol):
    ...
    async def initiate_transaction(self, file_name: str, mode: HeaderFlags) -> None:
        file_name_pkt = Packet(
            ...
            flags=HeaderFlags.GBN.value | mode.value,
        )
        retries = 0
        while retries < 3:
        ...
        try:
            match mode:
                case HeaderFlags.UPLOAD:
                    ...
                    self.current_file = file_manager.file
                    await self._send_ack(file_name_pkt.get_seq_num(), 1)
                ...
                case HeaderFlags.DOWNLOAD:
                    ...
                    self.current_file = file_manager.file
                    await self._send_ack(file_name_pkt.get_seq_num(), 0)
    ...
    async def _send_ack(self, ack_num: int, mode: int) -> None:
        ack = Packet(self.next_seq_num, ack_num, flags=HeaderFlags.GBN.value | mode)
        await self.socket.send(ack)
