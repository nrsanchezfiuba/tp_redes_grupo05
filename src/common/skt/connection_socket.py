...
class ConnectionSocket:
    @classmethod
    def for_client(
        cls, addr: Tuple[str, int], protocol: HeaderFlags
    ) -> "ConnectionSocket":
        return cls(addr, None, protocol)

    @classmethod
    async def for_server(
        cls, addr: Tuple[str, int], queue: asyncio.Queue[Packet], protocol: HeaderFlags
    ) -> "ConnectionSocket":
        return cls(addr, queue, protocol)
